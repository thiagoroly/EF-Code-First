    ...
    class Program
    {
        static void Main(string[] args)
        {
            using (var db = new MovieContext())
            {
                ...
                Console.ReadKey();
                var elenco = from mv in db.Movies
                             join am in db.Characters on mv.ID equals am.MovieId
                             join ac in db.Actors on am.ActorId equals ac.ActorId
                             where mv.Title == "Jurassic Park"
                             select new { am.Character, ac.Name };
                ...
                Console.ReadKey();
                var james = from am in db.Characters
                            join ac in db.Actors on am.ActorId equals ac.ActorId
                            where am.Character == "James Bond"
                            group ac by ac.Name;

                foreach (var j in james)
                {
